CREATE TABLE IF NOT EXISTS Category (
   ...
   CONSTRAINT proper_title CHECK ( Category.title ~* '^[A-Za-z0-9_]+$' ),
   CONSTRAINT proper_description CHECK ( Category.description ~* '^[A-Za-z0-9_]+$' )
...
CREATE TABLE IF NOT EXISTS Blog (
   ...
   is_public BOOLEAN NOT NULL DEFAULT '1',
   CONSTRAINT proper_title CHECK ( Blog.title ~* '^[A-Za-z0-9_]+$' ),
   CONSTRAINT proper_description CHECK ( Blog.description ~* '^[A-Za-z0-9_]+$' )
...
CREATE TABLE IF NOT EXISTS Comment (
    ...
    CONSTRAINT proper_username CHECK ( Comment.username ~* '^[A-Za-z0-9]+$' )
