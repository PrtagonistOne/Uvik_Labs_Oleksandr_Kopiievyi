...
@pytest.mark.django_db
def test_retrieve_post(crud_APIFactory):
    # given
...
@pytest.mark.django_db
def test_create_post(crud_APIFactory):
    # given
...
@pytest.mark.django_db
def test_put_post(crud_APIFactory):
    # given
...
@pytest.mark.django_db
def test_patch_post(crud_APIFactory):
    # given
    ...
    # then
    assert response.status_code == 200
    assert response.data['main_title'] == update_new_main_title


@pytest.mark.django_db
def test_delete_post(crud_APIFactory):
    # given
    ...
    expected_results = Post.objects.count()
    # then
    assert response.status_code == 204
    assert response.data is None
    assert old_post_count > expected_results
